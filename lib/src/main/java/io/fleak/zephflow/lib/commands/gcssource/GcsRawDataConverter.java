...
@Slf4j
public class GcsRawDataConverter implements RawDataConverter<GcsObjectData> {

  private final FleakDeserializer<?> fleakDeserializer;

  public GcsRawDataConverter(FleakDeserializer<?> fleakDeserializer) {
    this.fleakDeserializer = fleakDeserializer;
  }

  @Override
  public ConvertedResult<GcsObjectData> convert(
      GcsObjectData sourceRecord, SourceExecutionContext<?> sourceInitializedConfig) {
    try {
      ...
      log.error("Failed to deserialize GCS object: {}", sourceRecord.objectName(), e);
