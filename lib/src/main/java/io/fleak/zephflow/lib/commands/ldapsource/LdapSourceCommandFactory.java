...
public class LdapSourceCommandFactory extends SourceCommandFactory {
  @Override
  public OperatorCommand createCommand(String nodeId, JobContext jobContext) {
    JsonConfigParser<LdapSourceDto.Config> configParser =
        new JsonConfigParser<>(LdapSourceDto.Config.class);
    LdapSourceConfigValidator validator = new LdapSourceConfigValidator();
    return new LdapSourceCommand(nodeId, jobContext, configParser, validator);
  }
}
