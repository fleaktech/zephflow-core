...
@Slf4j
public class SyslogUdpCommand extends SimpleSourceCommand<SerializedEvent> {

  public SyslogUdpCommand(
      String nodeId,
  ...
  protected ExecutionContext createExecutionContext(
    ...
    SyslogUdpFetcher udpFetcher;
    try {
      udpFetcher = new SyslogUdpFetcher(config);
      udpFetcher.start();
    } catch (Exception e) {
      throw new RuntimeException("Failed to start UDP receiver", e);
    }

    try {
      Charset charset = Charset.forName(config.getEncoding());
      RawDataConverter<SerializedEvent> converter = new SyslogUdpRawDataConverter(charset);
      ...
      udpFetcher.close();
      throw new RuntimeException("Failed to initialize UDP source context", e);
    }
  }
  ...
  public String commandName() {
    return COMMAND_NAME_SYSLOG_UDP;
  }
}
