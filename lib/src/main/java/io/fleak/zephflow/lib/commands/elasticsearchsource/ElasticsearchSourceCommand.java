...
public class ElasticsearchSourceCommand extends SimpleSourceCommand<ElasticsearchDocument> {

  public ElasticsearchSourceCommand(
      String nodeId,
  ...
  public String commandName() {
    return COMMAND_NAME_ELASTICSEARCH_SOURCE;
  }
  ...
  protected ExecutionContext createExecutionContext(
    ...
    Fetcher<ElasticsearchDocument> fetcher = createFetcher(config, jobContext);

    FleakDeserializer<?> deserializer =
        DeserializerFactory.createDeserializerFactory(config.getEncodingType())
            .createDeserializer();
    RawDataConverter<ElasticsearchDocument> converter =
        new ElasticsearchRawDataConverter(deserializer);
    RawDataEncoder<ElasticsearchDocument> encoder = new ElasticsearchRawDataEncoder();
  ...
  private Fetcher<ElasticsearchDocument> createFetcher(
      ElasticsearchSourceDto.Config config, JobContext jobContext) {
    ...
    return new ElasticsearchSourceFetcher(
        config.getHost(),
        config.getIndex(),
        config.getQuery(),
