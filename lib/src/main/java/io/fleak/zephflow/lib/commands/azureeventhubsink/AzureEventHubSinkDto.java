/**
 * Copyright 2025 Fleak Tech Inc.
 *
 * <p>Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file
 * except in compliance with the License. You may obtain a copy of the License at
 *
 * <p>http://www.apache.org/licenses/LICENSE-2.0
 *
 * <p>Unless required by applicable law or agreed to in writing, software distributed under the
 * License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either
 * express or implied. See the License for the specific language governing permissions and
 * limitations under the License.
 */
package io.fleak.zephflow.lib.commands.azureeventhubsink;

import io.fleak.zephflow.api.CommandConfig;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;

/** Configuration for the Azure Event Hub sink connector. */
public interface AzureEventHubSinkDto {

  @Data
  @Builder
  @NoArgsConstructor
  @AllArgsConstructor
  class Config implements CommandConfig {

    // --- Event Hub connection / authentication (exactly one auth mode) ---

    /** SAS connection string for the Event Hub namespace or entity. */
    private String connectionString;

    /** Fully qualified namespace, e.g. {@code my-namespace.servicebus.windows.net} (Entra ID). */
    private String fullyQualifiedNamespace;

    /** Event Hub (topic) name to produce to. */
    private String eventHubName;

    // --- Optional Entra ID service principal; when omitted, DefaultAzureCredential is used ---
    private String tenantId;
    private String clientId;
    private String clientSecret;

    /**
     * Optional path expression whose value becomes the Event Hub partition key. Events sharing a
     * key land on the same partition.
     */
    private String partitionKeyFieldExpressionStr;

    /** Output encoding (e.g. {@code JSON_OBJECT}). */
    private String encodingType;
  }
}
