...
class ReaderConfigValidatorTest {

  private final ReaderConfigValidator validator = new ReaderConfigValidator();

  @Test
  void validateConfig_unsupportedEncodingType() {
    ReaderDto.Config config =
        ReaderDto.Config.builder()
            .path("s3://bucket/key")
  ...
  void validateConfig_allSupportedEncodingTypes() {
    for (EncodingType type : DeserializerFactory.SUPPORTED_ENCODING_TYPES) {
      ReaderDto.Config config =
          ReaderDto.Config.builder().path("s3://bucket/key").encodingType(type).build();
